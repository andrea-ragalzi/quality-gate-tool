...
async def test_live_watch_full_flow(tmp_path: Path):
    ...
    with patch.dict(
        ...
        init_calls = [c for c in notifier.send_update.call_args_list if c.args[1].get("type") == "GLOBAL_INIT"]
        end_calls = [c for c in notifier.send_update.call_args_list if c.args[1].get("type") == "GLOBAL_END"]
        ...
        assert len(incremental_init) >= 1, "Incremental analysis did not start after file modification"

        # Check for Module Execution
        module_init = [m for m in messages if m.get("type") == "INIT" and m.get("module") == "B_Ruff"]
